...
public class JdbcUtil {
    ...
    public static void getPrimaryKeysInfo() {  
        ...
        try{  
            ...
            while (rs.next()){  
                ...
                System.out.println("getPrimaryKeys---"+tableCat + " - " + tableSchemaName + " - " + tableName + " - " + columnName + " - "
                       + keySeq + " - " + pkName);     
            }  
        }catch (SQLException e){  
            e.printStackTrace();  
    ...
    public static void getIndexInfo() { 
        ...
        try{  
        	 ...
        	 */
            rs = dbmd.getIndexInfo("nutsdemo", null, "t_member", false, true);  
            while (rs.next()){  
            	String tableCat = rs.getString("TABLE_CAT");  //表类别(可为null) 
				String tableSchemaName = rs.getString("TABLE_SCHEM");//表模式（可能为空）,在oracle中获取的是命名空间,其它数据库未知     
				String tableName = rs.getString("TABLE_NAME");  //表名  
                ...
                System.out.println("getIndexInfo---"+tableCat + " - " + tableSchemaName + " - " + tableName + " - " + nonUnique + " - " 
...
//在获取元数据中的REMARK(即 备注)前，需要设置RemarksReporting属性为true，否则获取到的REMARK属性都是null，设置该属性有两种方法，请参阅上面的Demo。